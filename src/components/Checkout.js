    ...
    if (validateRequest(items, addresses)) {
      try {
        ...
        if(err.response && err.response.data) {
          ...
          enqueueSnackbar('Something went wrong while checking out', {variant: "error"})
        }
      }
      // return true;
    }
          ...
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Box>
              {addresses?.all.length ? (
                <>
                  {addresses.all.map((p, id) => {
                    return (
                      <Stack padding={2} spacing={2} key={id}>
                        <Box
                          key={id}
                          className={`address-item ${
                            addresses.selected === p._id
                              ? "selected"
                              : "not-selected"
                          }`}
                          onClick={() =>
                            setAddresses({ ...addresses, selected: p._id })
                          }
                        >
                          <span>{p.address}</span>
                          <Button variant="text" startIcon={<Delete/>} onClick={() => deleteAddress(token, p._id)}>
                            DELETE
                          </Button>
                        </Box>
                      </Stack>
